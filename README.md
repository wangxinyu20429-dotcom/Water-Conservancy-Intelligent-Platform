...
仓库包含可安装的 [`hydrology-evidence-cards`](skills/hydrology-evidence-cards/SKILL.md) Skill。它从本机 Zotero 只读取得指定来源和可用全文，按 Source—Claim—Decision 三层建立证据：来源版本只登记一次，Cxx 声明带全局 ID 和原文定位，平台判断写入第九张决策卡。V1.6要求沿文献自己的科学逻辑写清研究对象、真实数据、分析单位、方法转换、比较与验证、关键结果、科学贡献和结论边界；读取主文的实质证据卡不得少于5,000个非空白字符，其中L1可见科学分析不得少于3,000字符。模板只负责检查遗漏，不能替作者生成固定话术。R09还会拦截重复段落、空标题和通用填充语。普通文献卡不再设置“复现需要什么”栏目，数据或代码可用性只在影响证据强度时说明。详细合同见[human-readable-core.md](skills/hydrology-evidence-cards/references/human-readable-core.md)，原V1.2架构说明仍见[证据卡科学架构与 Zotero Skill 使用 V1.2](docs/证据卡规范/07_证据卡科学架构与Zotero_Skill使用_V1.2.md)。
## 证据卡到主题 Skill
...
Zotero“科研平台”集合42个唯一来源各有一张独立证据卡，并由主题Skill形成5个候选主题。2026-09-09已按V1.6加深：取得Zotero索引正文的卡片分别交代研究问题、数据构成、独立分析单位、方法链、对照或验证、承重结果与贡献，综述则交代检索范围、证据构成、综合方法和异质性；没有正文的官方网页和题录来源停在材料实际支持的范围。有主文内容的38张卡均达到不少于5,000个非空白字符的来源实质内容门槛，读者可见部分逐篇补入来源特定的深度分析；机器校验字段继续默认折叠。全部仍待课题组成员逐表逐图复核，因此候选主题只用于确定精读顺序和补证据任务，不能直接作为论文结论或创新性声明。
