# Human-readable scientific core V1.3
...
Depth comes from reconstructing the evidence path, not from word count. A substantive full-text original study normally needs at least 2,500 visible Chinese characters to cover the core and often 6,000–10,000 for L2. Reviews and major investigation reports may need more. Short official notices may be much shorter if the card explicitly states that no data, method or effect result exists. Repetition, generic domain background, raw abstracts and machine fields do not count.
