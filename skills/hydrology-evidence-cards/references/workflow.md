...
For a substantive full-main-text original research paper, target 4,500–7,000 Chinese characters for L1 and 6,000–10,000 for L2 in the visible analysis. Count only the research narrative, not frontmatter, links, repeated metadata, calibration notes or machine fields. Include one explicit `全文证据链展开` that joins named inputs, transformations, checks, results and boundaries. When the source does not contain enough substance, write a shorter card and state the source-level reason; never expand by paraphrasing the same conclusion repeatedly.
