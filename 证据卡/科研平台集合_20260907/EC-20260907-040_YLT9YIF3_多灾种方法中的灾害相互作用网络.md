...
> **初读结论：** 论文区分单灾种叠加与真正包含相互作用的多灾种方法，提出触发、概率增加和催化／阻滞三类关系，并用矩阵和流程图表示级联网络。
...
作者综合文献、现场观察和半结构访谈，定义自然灾害、人为过程和技术灾害三组对象，整理三类交互关系，并用案例构建灾害交互矩阵与灾害／过程流程图。

## 初读抓到的主要结果或观点

相互作用可能连接成链或网络，并在连续冲击间改变社会、基础设施和应对能力。作者认为把这些关系纳入方法有助于更接近真实事件、约束动态脆弱性并改进减灾资源排序。
...
精读关系判据、矩阵和流程图的证据来源；比较2019年五类关系，形成术语映射而非简单合并。

**候选主题信号：** 多灾种方法、灾害级联、交互矩阵、动态脆弱性。这些标签只用于后续聚合，不代表主题已经成立。
